	...
	public async startGame(interaction: ChatInputCommandInteraction<"cached">) {
		...
		try {
			...
			collector.on("collect", async message => {
				...
				else if (this._isStarted && !isValid)
					return collector.stop(BOT_WIN);
				...
				if (!this._isStarted) {
					...
					const lastWord =
						this._usedWords[this._usedWords.length - 1];
					const lastChar = lastWord.charAt(lastWord.length - 1);
					const checkLastChar = new RegExp(
						`^[${lastChar}${dueum(lastChar)}]`,
					);
